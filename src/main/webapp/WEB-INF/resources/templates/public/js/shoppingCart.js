...
var shoppingCart = (function () {
    ...
    function Item(name, price, count, image, productcode, isCombo, storeId) {
        this.name = name
        this.price = price
        this.count = count
        this.image = image
    ...
    obj.addItemToCart = function (name, price, count, image, productcode, isCombo, storeId) {
        var qtyItem = $("#product-qty-" + productcode).val();
        for (var i in cart) {
            if (cart[i].productcode === productcode) {
                if (cart[i].count + count > qtyItem) {
        ...
        console.log("addItemToCart:", name, price, count, image, productcode, isCombo, storeId);

        var item = new Item(name, price, count, image, productcode, isCombo, storeId);
        cart.push(item);
        saveCart();
    };
...
$(".add-item-to-cart").click(function (event) {
    if (localStorage.getItem("userInfo"))
    {
        ...
        var isCombo = $(this).attr("data-is-combo");
        var storeId = $(this).attr("data-store");

        shoppingCart.addItemToCart(name, price, 1, nameimages, productcode, isCombo , storeId);
        displayCart();
        displayCartPageCart();
    } else {
        $("#myModal").modal();
